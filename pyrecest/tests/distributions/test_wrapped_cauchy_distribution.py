...
class WrappedCauchyDistributionTest(unittest.TestCase):
    ...
    def test_pdf(self):
        ...
        np.testing.assert_allclose(
            custom_with_formular.pdf(xs=self.xs),
            custom_wrapped.pdf(self.xs),
            atol=0.0001,
        )
        np.testing.assert_allclose(
            custom_wrapped.pdf(xs=self.xs), custom_wrapped.pdf(self.xs)
        )

    def test_cdf(self):
        np.testing.assert_allclose(
            self.dist.cdf(np.array([1])), self.dist.integrate(np.array([0, 1]))
        )
